<template>
  <div>
    ...
    >
      <g opacity="0.9">
        <circle cx="24" cy="24" r="24" :fill="isLike ? '#e91e63' : '#F1F1F2'" />
    ...
    <span class="numbers">{{ formatSuffixNumber(totalLikes) }}</span>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      totalLikes: this.video.total_likes,
      isLike: false,
    };
  },
  methods: {
    ...
    async handleLikeVideo() {
      try {
        ...
        //   this.totalLikes = (parseInt(this.totalLikes) + 1).toString();
        // } else this.totalLikes = (this.totalLikes - 1).toString();
      } catch (error) {
        if (error.response) {
          alert(error.response.data);
        }
